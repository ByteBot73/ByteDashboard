...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const res = await fetch('https://thenewdashboard.onrender.com/api/guilds', { credentials: 'include' });
  ...
  if (data.guilds && data.guilds.length) {
    data.guilds.filter(guild => guild.owner).forEach(guild => {
      ...
      if (guild.botInGuild) {
        ...
        btn.onclick = async () => {
          fetch('https://thenewdashboard.onrender.com/api/invite').then(r => r.json()).then(invite => {
            window.open(invite.invite + `&guild_id=${guild.id}&disable_guild_select=true`, '_blank');
            // Poll for bot presence and update button
            const poll = setInterval(async () => {
              const guildsRes = await fetch('https://thenewdashboard.onrender.com/api/guilds', { credentials: 'include' });
